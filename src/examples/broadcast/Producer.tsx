...
export default function ProducerView() {
  ...
  const startStream = async () => {
    ...
    socket.on("consumer", async (payload) => {
      ...
      stream
        .getTracks()
        .forEach((track: any) => peers[payload.id].addTrack(track, stream));
      ...
      peers[payload.id].ontrack = ({ streams }: { streams: any }) => {
        ...
        return createVideoElement({
          id: payload.id,
          stream: new MediaStream(streams[0]),
        });
      };
      ...
      peers[payload.id].onconnectionstatechange = (event: any) => {
        ...
        if (
          peers[payload.id].connectionState === "failed" ||
          peers[payload.id].connectionState === "disconnected"
        ) {
    ...
    socket.on("candidate", (payload) => {
      if (payload.candidate) {
        ...
        peers[payload.id].addIceCandidate(
